// Vendored from https://tweetnacl.cr.yp.to/ — Public domain
// TweetNaCl v20140427 by Daniel J. Bernstein, Bernard van Gastel,
// Wesley Janssen, Tanja Lange, Peter Schwabe, Sjaak Smetsers

#include "tweetnacl.h"
#include <string.h> /* for memcpy */
#include <stdlib.h> /* for malloc/free in detached verify wrapper */
...
{
  unsigned long long sm_len = msg_len + 64;
  unsigned long long mlen;
  int ret;
  int heap = (sm_len > 2048);

  unsigned char sm_buf[2048], m_buf[2048];
  unsigned char *sm = heap ? (unsigned char *)malloc(sm_len) : sm_buf;
  unsigned char *m  = heap ? (unsigned char *)malloc(sm_len) : m_buf;
  ...
  /* NaCl combined format: sig (64 bytes) || msg */
  memcpy(sm, sig, 64);
  memcpy(sm + 64, msg, msg_len);

  ret = crypto_sign_open(m, &mlen, sm, sm_len, pk);
