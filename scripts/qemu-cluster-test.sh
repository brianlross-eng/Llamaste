...
# Find a second free port by temporarily bumping the base past PORT1
PORT2=$(QEMU_PORT_BASE=$((PORT1 + 1)) find_free_port)
...
STATUS1=$(http_get "http://localhost:${PORT1}/llamaste/cluster/status")
STATUS2=$(http_get "http://localhost:${PORT2}/llamaste/cluster/status")
...
# -------------------------------------------------------
info "Test 2: POST /llamaste/cluster/add-peer (API contract)"
...
ADD_RESP=$(http_post "http://localhost:${PORT1}/llamaste/cluster/add-peer" \
...
# -------------------------------------------------------
# Test 4: GET /llamaste/cluster/capacity
# -------------------------------------------------------
info "Test 4: GET /llamaste/cluster/capacity"

CAP1=$(http_get "http://localhost:${PORT1}/llamaste/cluster/capacity")
CAP2=$(http_get "http://localhost:${PORT2}/llamaste/cluster/capacity")
...
# -------------------------------------------------------
# Test 5: GET /llamaste/cluster/models
# -------------------------------------------------------
info "Test 5: GET /llamaste/cluster/models"

MODELS1=$(http_get "http://localhost:${PORT1}/llamaste/cluster/models")
MODELS2=$(http_get "http://localhost:${PORT2}/llamaste/cluster/models")
...
# -------------------------------------------------------
# Test 6: GET /llamaste/cluster/peers returns JSON array
# -------------------------------------------------------
info "Test 6: GET /llamaste/cluster/peers"

PEERS1=$(http_get "http://localhost:${PORT1}/llamaste/cluster/peers")
PEERS2=$(http_get "http://localhost:${PORT2}/llamaste/cluster/peers")

# Peers response may be a bare array or an object with .peers array
if echo "$PEERS1" | jq -e '.peers // . | type == "array"' >/dev/null 2>&1; then
    pass "Node 1 /llamaste/cluster/peers returns valid response"
else
    fail "Node 1 /llamaste/cluster/peers invalid: ${PEERS1}"
fi

if echo "$PEERS2" | jq -e '.peers // . | type == "array"' >/dev/null 2>&1; then
    pass "Node 2 /llamaste/cluster/peers returns valid response"
else
    fail "Node 2 /llamaste/cluster/peers invalid: ${PEERS2}"
fi

# -------------------------------------------------------
# Test 7: POST /llamaste/cluster/reload returns new_role
# -------------------------------------------------------
info "Test 7: POST /llamaste/cluster/reload"

RELOAD1=$(http_post "http://localhost:${PORT1}/llamaste/cluster/reload" '{}')
RELOAD2=$(http_post "http://localhost:${PORT2}/llamaste/cluster/reload" '{}')
